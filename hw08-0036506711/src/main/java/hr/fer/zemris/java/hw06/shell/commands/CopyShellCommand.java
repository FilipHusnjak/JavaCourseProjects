...
public class CopyShellCommand implements ShellCommand {
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		try (InputStream is = Files.newInputStream(env.getCurrentDirectory().resolve(list.get(0))); 
				OutputStream os = Files.newOutputStream(getDestinationPath(env.getCurrentDirectory().resolve(list.get(1)), 
						Paths.get(list.get(0)), env))) {
