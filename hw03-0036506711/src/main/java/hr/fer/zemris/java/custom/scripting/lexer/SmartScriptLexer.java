 ...
 * numbers, strings, symbols and functions. If the current state is set to {@code SmartScriptLexerState.DOCUMENT_TEXT} it can only
 * tokenize text. SmartScriptLexerState can be set using the proper method.
 * 
...
public class SmartScriptLexer {
	...
	/**
	 * This method is used to tokenize expression when {@code SmartScriptLexerState} is set to {@code SmartScriptLexerState.DOCUMENT_TEXT}.
	 * <br>
	 * Method returns the tokenized element as {@code SmartScriptToken} or throws {@code SmartScriptLexerException} if the element
	 * cannot be tokenized. It reads text until first valid open tag appears. Only backslash
	...
	/**
	 * This method is used to tokenize expression when {@code SmartScriptLexerState} is set to {@code SmartScriptLexerState.INSIDE_TAG}.
	 * <br>
	 * It returns the tokenized element as {@code SmartScriptToken} or throws {@code SmartScriptLexerException} if the element
	 * cannot be tokenized. In this state as next element it can return: variable, string, function, symbol,
	...
	private SmartScriptToken insideTagLexerState() {
		...
		if (data[currentIndex] == '@') {
			if (!Character.isLetter(data[++currentIndex])) {
				throw new SmartScriptLexerException("Invalid function name! Function names should start with a letter!");
			}
	 ...
	 * quotation mark is read. Only quotation marks, backslash or ('\n', '\t', '\r') escapes are possible,
	 * otherwise this method throws {@code SmartScriptLexerException}.
	 * 
	...
	private void escapeBracketOrBackslash() {
		...
		if (currentIndex >= data.length || (data[currentIndex] != '{' && data[currentIndex] != '\\')) {
			throw new SmartScriptLexerException("Escaping this character in document text is prohibited! Char: " + data[currentIndex]);
		}
	}
	...
	private void escapeQuotationsOrBackslash() {
		...
		if (currentIndex >= data.length || (data[currentIndex] != '"' && data[currentIndex] != '\\' && data[currentIndex] != 'n' && data[currentIndex] != 'r' && data[currentIndex] != 't')) {
			throw new SmartScriptLexerException("Escaping this character in string is prohibited! Char: " + data[currentIndex]);
		}
	}
	...
	private SmartScriptToken extractNumber() {
		...
		try {
			...
			try {
				return new SmartScriptToken(SmartScriptTokenType.CONSTANT_DOUBLE, Double.parseDouble(num));
			} catch (NumberFormatException ex2) {
				throw new SmartScriptLexerException("Given number is not possible to convert to Integer or Double! Number: " + num);
			}
		}
	}
